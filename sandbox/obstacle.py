...
class CircularObstacle(object):
    ...
    def getCostGradient(self, robotstate, robotradius):
        ...
        #checking if the obstacle and the circle the robot is bounded by intersect
        if(d < (self._radius + robotradius)):
            grad = self._penalty * np.array([-(math.e ** (-d))*(x)/d, -(math.e ** (-d))*(y)/d])
            return grad
        else:
            return np.zeros(6)
    ...
    def getCostHessian(self, robotstate, robotradius):
        ...
        if(d < (self._radius + robotradius)):
            h = np.zeros((6,6))
            h[0][0] = ((math.e**(-d))*(x**2)*d - (math.e**(-d))*(y**2))/(d**3)
            h[0][1] = x*((math.e**(-d))*y*d + (math.e**(-d))*y)/(d**3)
            h[1][0] = x*((math.e**(-d))*y*d + (math.e**(-d))*y)/(d**3)
            h[1][1] = ((math.e**(-d))*(y**2)*d - (math.e**(-d))*(x**2))/(d**3)
